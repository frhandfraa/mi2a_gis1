...
===================================================== */
$pilihanJurusan = $zonasi['pilihan_jurusan'] ?? [];

function normalisasiPilihan($pilihan) {
    return [
        'sekolah' => $pilihan['sekolah']
            ?? $pilihan['nama_smk']
            ?? '-',
...
$pilihan1 = normalisasiPilihan($pilihanJurusan['pilihan1'] ?? []);
$pilihan2 = normalisasiPilihan($pilihanJurusan['pilihan2'] ?? []);
...
<h4 class="fw-bold mb-4">6️⃣ Finalisasi Pendaftaran Jalur Prestasi</h4>
...
<span class="text-muted">Jurusan: <?= htmlspecialchars($pilihan1['jurusan']) ?></span>
</p>
...
<p class="text-muted">
Rata-rata Semester:
<strong><?= number_format($rata_per_semester[$semester] ?? 0, 2) ?></strong>
</p>
<?php endforeach; ?>
<?php else: ?>
...
<p>
<strong>Rata-rata Total Rapor:</strong>
<span class="badge bg-success fs-6"><?= number_format($rata_total, 2) ?></span>
</p>
...
<?= !empty($dokumen[$key])
    ? "<a href='../../uploads/zonasi/{$dokumen[$key]}' target='_blank'>Lihat Dokumen</a>"
...
<form action="step8_prestasi_kunci.php" method="post">
